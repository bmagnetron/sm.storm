	...
	public void execute(Tuple tuple) {
		...
		//	URLConnection conn = url.openConnection();
			...
			if (200 <= conn.getResponseCode() && conn.getResponseCode() <= 299) {
				...
				/* to fetch every 1 sec from the url otherwise it will fetch fro every millisec */
				
				try {
				Thread.sleep(1000);

				GAPI_MAP obj = mapper.readValue(conn.getInputStream(), GAPI_MAP.class);
				ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream("c:\\temp\\stock1.txt"));
				oos.writeObject(obj);
